...
export const Loginform = () => {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://www.indiadealss.com/api/auth/send-otp", { mobile });
      if (res.data.success) {
        setOtpSent(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "login") {
        const res = await axios.post("https://www.indiadealss.com/api/auth/login", {
          email,
          password,
        });
        if (res.data.success) {
      ...
      if (mode === "register") {
        const res = await axios.post("https://www.indiadealss.com/api/auth/register", {
          name,
          mobile,
          email,
          password,
      ...
      if (mode === "forgot") {
        const res = await axios.post("https://www.indiadealss.com/api/auth/forgot-password", { email });
        if (res.data.success) {
      ...
      if (mode === "loginWithMobile") {
        const res = await axios.post("https://www.indiadealss.com/api/auth/verify-otp", {
          mobile,
          otp,
        });
        if (res.data.success) {
    ...
    <>
          ...
          <div className="mb-3">
            <label for="email-address-icon" className=' block mb-2 text-sm font-medium text-gray-900 dark:text-white'>Full Name</label>
                <div className='flex'>
                    ...
                    <input type="text" value={name} id="website-admin" onChange={(e) => setName(e.currentTarget.value)} className='rounded-none rounded-e-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'placeholder="Full Name" />
                </div>
          </div>
          <div className="mb-3">
      ...
      {(mode === "login" || mode === "register" || mode === "forgot") && (
        <div className="mb-3">
          ...
          <label for="email-address-icon" className='my-4 block mb-2 text-sm font-medium text-gray-900 dark:text-white'>Email</label>
      ...
      {mode !== "forgot" && mode !== "loginWithMobile" && (
        <div className="mb-3">
          <label for="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                    <input type="password" id="Password" value={password} onChange={(e) => setPassword(e.currentTarget.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="•••••••••" required />
