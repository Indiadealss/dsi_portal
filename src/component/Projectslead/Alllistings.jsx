...
const Alllistings = () => {
    ...
    useEffect(() => {
        ...
        const fetchProperties = async () => {
            try {
                const res = await getAllpropertiesDetailsUser(user.id, page, limit, status);
            ...
            <div className='shadow-md w-[96%] m-3 p-2 border border-gray-300 rounded'>
                <div className='flex'>
                    ...
                    <div className='flex items-center gap-2 ms-10'>
                        ...
                        {category !== "All" && (
                            <button className="text-xs bg-blue-100 px-2 py-1 rounded-full cursor-pointer" onClick={() => setCategory("All")}>
                                {category} <RxCross2 className='m-1 text-sm text-blue-500'/>
                            </button>
                        )}

                        {search && (
                            <button className="text-xs bg-blue-100 px-2 py-1 rounded-full cursor-pointer" onClick={() =>  setSearch("")}>
